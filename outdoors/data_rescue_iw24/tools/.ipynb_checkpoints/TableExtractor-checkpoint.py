...
class TableExtractor:
    ...
    def find_with_min_area(self, threshold_start = 30, threshold_end=150, area_percentage_min=0.5, area_percentage_max=0.85):
        ...
        self.convolve_image()
        self.store_process_image("2_convolved.jpg", self.convolved_image)
        #self.find_horizontal_mean()

        while self.area_percentage < self.area_percentage_min:

            self.threshold_image()
            self.store_process_image(f"3_thresholded_{self.threshold}.jpg", self.thresholded_image)
            self.invert_image()
            self.store_process_image(f"4_inverteded_{self.threshold}.jpg", self.inverted_image)
            self.dilate_image()
            self.store_process_image(f"5_dialateded_{self.threshold}.jpg", self.dilated_image)
            self.find_contours()
            self.store_process_image(f"6_all_contours_{self.threshold}.jpg", self.image_with_all_contours)
            self.filter_contours_and_leave_only_rectangles()
            self.store_process_image(f"7_only_rectangular_contours_{self.threshold}.jpg", self.image_with_only_rectangular_contours)
            self.find_largest_contour_by_area()
            self.store_process_image(f"8_contour_with_max_area_{self.threshold}.jpg", self.image_with_contour_with_max_area)
        ...
        self.store_process_image(f"9_with_4_corner_points_plotted_{self.threshold}.jpg", self.image_with_points_plotted)
        self.calculate_new_width_and_height_of_image()
        self.apply_perspective_transform()
        self.store_process_image(f"10_perspective_corrected_{self.threshold}.jpg", self.perspective_corrected_image)
    ...
    def convolve_image(self):
        ...
        image_horizontal_convolve = np.convolve(image_horizontal_mean, np.ones(mean_width)/mean_width, mode='same')
        image_horizontal_corrected = self.grayscale_image[mean_width:self.grayscale_image.shape[0]-mean_width] / image_horizontal_convolve
        ...
        image_vertical_convolve = np.convolve(image_vertical_mean, np.ones(mean_width)/mean_width, mode='same')
        cropped_image = image_horizontal_corrected[:, mean_width:image_horizontal_corrected.shape[1]-mean_width]
        ...
        self.image = self.image[mean_width:self.image.shape[0]-mean_width, mean_width:self.image.shape[1]-mean_width, :]

    def find_horizontal_mean(self):

        image_horizontal_mean = np.mean(self.convolved_image, axis=0)
        image_mean = np.mean(image_horizontal_mean)
        image_std = np.std(image_horizontal_mean)

        plt.plot(image_horizontal_mean)
        plt.plot([image_mean+image_std]*len(image_horizontal_mean))
        plt.plot([image_mean-image_std]*len(image_horizontal_mean))
        plt.show()
        print()
    ...
    def threshold_image(self):
        ...
        blur = cv2.GaussianBlur(self.convolved_image,(5,5),0)
